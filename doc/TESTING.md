...
All test scripts live in `src/backend/test/` and expect real request data at `testdata/request.json` (repo root, git-ignored — contains real member data, not committed). The file must have the shape described in `schemas/driving_plan_request.json`.

## 1. Solver determinism test (no server needed)

Replays a real capture from `src/backend/src/captures/` through `SolverService`
...
```bash
python src/backend/test/test_determinism_solver.py
```
...
# Terminal 2
python src/backend/test/test_integration.py
```
...
- No member drives more than their configured max (see `MAX_DRIVES_FULLTIME` / `MAX_DRIVES_PARTTIME` in `src/backend/src/config.py`).
