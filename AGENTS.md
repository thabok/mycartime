...
- `src/frontend/` is a **git submodule** pointing at a separate repository
  ...
  - To change code in it: edit files under `src/frontend/` normally, then
    commit *inside* it (`git -C src/frontend add -A && git -C src/frontend
    commit`). The superproject only tracks which commit hash the submodule
    points at — after committing inside the submodule, `git add src/frontend`
    in the superproject to record the new pointer.
  - Do not `git add`/commit individual files under `src/frontend/` from the
  ...
  - Pushing submodule commits requires pushing from inside the submodule
    (`git -C src/frontend push origin main`) — the superproject's own push
    does not push submodule commits.
- Backend code under `src/backend/src/` uses **flat imports** (`import
  config`, `from solver_service import ...`), so it must be run with
  `src/backend/src` as the working directory / on `sys.path`, e.g. `cd
  src/backend/src && python app.py`. Running it as `python -m src.app` from
...
- `./scripts/run.sh` from repo root starts both backend (port 1338) and
  frontend (port 8080, Vite, hot reload) together. Ctrl+C stops both.
- Backend has no auto-reload in this setup; after backend code changes,
  restart `./scripts/run.sh` (or just the backend process) to pick them up.
- Backend tests: `source .venv/bin/activate && cd src/backend && python -m
...
`src/backend/src/solver_service.py` (see `doc/ALGORITHM_EVOLUTION.md`) usually
...
plan generation in a batch (e.g. `src/backend/src/experiments/run_batch.py`, or
...
There is no third-party WebUntis library. `src/backend/src/webuntis_client.py`
...
- Don't add comments explaining *what* code does; only note non-obvious
  *why* (as in the rest of this file and `scripts/run.sh`).
